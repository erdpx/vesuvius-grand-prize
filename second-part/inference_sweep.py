...
def list_checkpoint_files(checkpoint_dir):
    """
    List all checkpoint files in the given directory.
    """
    return [f for f in os.listdir(checkpoint_dir) if f.endswith('.ckpt')]

def run_inference_for_checkpoint(checkpoint_path, args, device):
    """
    Run inference for a single checkpoint file.
    """
    print(f"Running inference for checkpoint: {checkpoint_path}")

    # Extracting the epoch information from the checkpoint filename
    epoch_info = re.search(r'epoch=\d+', checkpoint_path)
    epoch_suffix = epoch_info.group(0) if epoch_info else 'unknown_epoch'

    model = RegressionPLModel.load_from_checkpoint(checkpoint_path, strict=False)
    model.to(device)
    model.eval()

    test_loader, test_xyxz, test_shape, fragment_mask = get_img_splits(
        args.segment_id, args.start_idx, args.start_idx + 30, 0
    )
    ...
    # Use the extracted epoch information in the output file name
    output_path = f'{args.out_path}/{args.segment_id}_{args.stride}_{args.start_idx}_{epoch_suffix}.png'
...
for checkpoint_file in checkpoint_files:
    ...
    run_inference_for_checkpoint(checkpoint_path, args, device)
